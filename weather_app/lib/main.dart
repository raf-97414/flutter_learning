...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // Lightweight service instances used only by this screen for searching and one-off fetches
  late final GeolocationServices _geolocation;
  late final WeatherService _weatherService;
  late final CacheServices _cacheService;
  ...
  void initState() {
    super.initState();
    _geolocation = GeolocationServices(Dio());
    _weatherService = WeatherService(Dio());
    _cacheService = CacheServices();

    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Load cached or network weather using provider
  ...
  void _onSearchChanged(String query) async {
    ...
    try {
      final results = await _geolocation.searchCities(query.trim());
      if (mounted) {
        setState(() {
  ...
  Future<void> _onCitySelected(CityResult city) async {
    ...
    try {
      // fetch weather for selected city coordinates, save to cache and tell provider to reload
      ...
      await _cacheService.saveCurrentWeather(fresh);
      // Tell provider to load (it will pick up cached value quickly)
